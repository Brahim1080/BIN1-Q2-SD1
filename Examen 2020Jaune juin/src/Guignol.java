...
public class Guignol {
    ...
    /**
     * initialise un spectacle de marionnettes
     *
     ...
     */
    public Guignol(int nombreTotalPlaces, int nombrePlacesEnfants) {
    ...
    public boolean reserverAdulte(String nom, ArrayList<Integer> numerosPlaces) {
        ...
        for (Integer place : numerosPlaces) {
            //>= car lemplacement 9 est la 10ieme place du tableau
            if (place < premPlaceAdulte || place >= tablePlaces.length || tablePlaces[place] != null )
                return false;
        }
        //Elle sont toutes vides => lajout peut se faire
        for (Integer place : numerosPlaces) {
    ...
    public int attribuerAutomatiquementPlace(String nom) {
        ...
        if (filePlacesLibres.isEmpty())
            return -1;
        int placeAttribue = filePlacesLibres.pop();
        tablePlaces[placeAttribue] = nom;
        return placeAttribue;
    }
