...
public class FileDePrioriteDEntiersImpl implements FileDePrioriteDEntiers {
	...
	private void pushUp(int i){
		...
		//indice 0 = racine donc pas de parent
		if (i == 0)
			return;
		// Bien palce
		if (arbre[i]<arbre[indiceParent(i)])
			return;
		System.out.println("indice arbre = " + i);
		int temp = arbre[indiceParent(i)];
		arbre[indiceParent(i)] = arbre[i];
		arbre[i] = temp;

		System.out.println("arbre[indiceParent(i)] = " + arbre[indiceParent(i)]);
		System.out.println("arbre[i] = " + arbre[i]);
