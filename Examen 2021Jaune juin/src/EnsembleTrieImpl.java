...
public class EnsembleTrieImpl<E> implements EnsembleTrie<E>{
	...
	public E elementLePlusGrand() {
		...
		return elementLePlusGrand(racine);
	}
	private E elementLePlusGrand(Noeud noeud) {
		if(noeud.droit == null)
			return noeud.element;
		//Lisez bien les explications pour l'utilisation de la methode compareTo() dans l'enonce
		return elementLePlusGrand(noeud.droit);
	}
	...
	public boolean ajoute(E element) {
		...
		//Lisez bien les explications pour l'utilisation de la methode compareTo() dans l'enonce
		return ajoute(racine,element,racine);
	}
	private boolean ajoute(Noeud noeud , E element , Noeud noeudParent){
		if (noeud == null){
			taille++;
			noeud = new Noeud(element);
			if(((Comparable<E>)element).compareTo(noeudParent.element)<0){
				noeudParent.gauche = noeud;
			}else noeudParent.droit = noeud;
			return true;

		}
		E elementNoeud = noeud.element;
		if(((Comparable<E>)elementNoeud).compareTo(element)<0)
			return ajoute(noeud.droit , element , noeud);

		if(((Comparable<E>)elementNoeud).compareTo(element)>0)
			return ajoute(noeud.gauche ,  element , noeud);
