...
public class EuroMatch {
	...
	public boolean placerDansFileAttente(String client){
		...
		if (mapClientReservation.containsKey(client)){
			if(mapClientReservation.get(client).getNombrePlacesReservees() ==MAX_PLACES_CLIENT)
				return false;
		}
	...
	public boolean passerNouvelleReservation(String client, int nombrePlacesDemandees){
		if (client == null || client.equals("") || nombrePlacesDemandees<= 0)
			throw new IllegalArgumentException();
		if (mapClientReservation.containsKey(client))
			return false;
		if (nombrePlacesDemandees> MAX_PLACES_CLIENT || nombrePlacesDemandees> nombrePlacesRestantes)
			return false;
	...
	public boolean modifierReservation(String client,int nombrePlacesDemandeesEnPlus){
		...
		if (!mapClientReservation.containsKey(client))
			return false;
		Reservation reservationDuClient = mapClientReservation.get(client);
		int nombreDePlaceClient = reservationDuClient.getNombrePlacesReservees();
		if (nombreDePlaceClient + nombrePlacesDemandeesEnPlus > MAX_PLACES_CLIENT)
			return false;
		reservationDuClient.setNombrePlacesReservees(nombreDePlaceClient+ nombrePlacesDemandeesEnPlus);
		return true;
	}	
