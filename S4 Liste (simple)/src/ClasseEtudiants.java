...
public class ClasseEtudiants{
	...
	public SerieEtudiants getSerieEtudiants(int numeroSerie)throws IllegalArgumentException{
		if (numeroSerie<1 || numeroSerie > nombreSeries())
			throw new IllegalArgumentException();
	...
	public boolean changerSerie(String nom, int nouveauNumeroSerie){
		if (nom == null || nouveauNumeroSerie<1 || nouveauNumeroSerie > nombreSeries())
			throw new IllegalArgumentException();
		int numeroSerie = numeroSerie(nom);

		if (numeroSerie==-1 || numeroSerie==nouveauNumeroSerie)
			return false;
		//ajoute dans la nouvelle
		tableSeries[nouveauNumeroSerie-1].ajouterEtudiant(nom);

		//retire de l'ancienne
		tableSeries[numeroSerie-1].supprimerEtudiant(nom);
		return true;
